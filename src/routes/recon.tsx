  ...
  head: () => ({
    meta: [
      { title: "Advanced Recon (OSINT) — Deep Eye" },
      {
        name: "description",
        content:
          "Passive OSINT, subdomain discovery, WHOIS, DNS, tech-stack fingerprint, and leaked credentials.",
      },
    ],
  }),
  component: Recon,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground font-mono">
          passive intelligence · zero-touch fingerprinting · target:{" "}
          <span className="text-emerald">acme.io</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <Card icon={Server} title="Tech Stack" sub="fingerprint">
          <div className="flex flex-wrap gap-1.5 mt-1">
            {TECH.map((t) => (
              <span
                key={t}
                className="px-2 py-0.5 rounded border border-emerald/30 text-emerald font-mono text-[11px]"
              >
                {t}
              </span>
            ))}
        ...
        <Card
          ...
          sub="haveibeenpwned + paste sites"
          tone="critical"
        >
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {SUBDOMAINS.map((s, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full ${danger ? "bg-[var(--severity-critical)]" : "bg-emerald"} pulse-dot`}
                />
                {s}
              </div>
            );
...
function KV({ k, v, tone }: { k: string; v: string; tone?: string }) {
  ...
  return (
    <div className="flex justify-between font-mono text-[12px] border-b border-border/30 pb-1 last:border-0">
      ...
      <span style={{ color: c }} className="text-right ml-2 truncate">
        {v}
      </span>
    </div>
  );
