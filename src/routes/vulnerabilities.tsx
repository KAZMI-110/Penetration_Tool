  ...
  head: () => ({
    meta: [
      { title: "Vulnerability Modules — Deep Eye" },
      {
        name: "description",
        content:
          "Interactive vulnerability board categorized by severity with API, GraphQL, and WebSocket modules.",
      },
    ],
  }),
  component: Vulns,
...
function Vulns() {
  ...
  const filtered = useMemo(() => {
    ...
    return findings.filter((f: any) => {
      ...
      switch (filter) {
        case "API Security":
          return m.includes("api") || m.includes("idor") || m.includes("cors");
        case "GraphQL":
          return m.includes("graphql");
        case "WebSocket":
          return m.includes("websocket");
        case "Auth/JWT":
          return m.includes("jwt") || m.includes("auth");
        case "Crypto":
          return m.includes("tls") || m.includes("crypto");
        case "Business Logic":
          return m.includes("logic") || m.includes("idor");
        default:
          return true;
      }
    });
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-md font-mono text-[11px] border transition ${
              filter === f
                ? "bg-emerald/15 text-emerald border-emerald/40 glow-emerald"
        ...
        {counts.map(({ s, n }) => (
          <div key={s} className="glass rounded-lg p-3 flex flex-col min-h-[400px]">
            ...
            <div className="space-y-2 overflow-y-auto">
              {filtered
                ...
                .map((f) => (
                  ...
                  >
                    ...
                    <div className="font-mono text-[10px] text-emerald mt-1 truncate">
                      {f.endpoint}
                    </div>
              ...
              {n === 0 && (
                <div className="font-mono text-[11px] text-muted-foreground/60 text-center py-6">
                  no findings
                </div>
              )}
            </div>
              ...
              <SheetHeader>
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{ background: severityColor(sel.severity) }}
                  />
                  <span
                    className="font-mono text-[11px]"
                    style={{ color: severityColor(sel.severity) }}
                  >
                    {sel.severity}
                  </span>
                ...
                <SheetDescription className="font-mono text-emerald">
                  {sel.endpoint}
                </SheetDescription>
              </SheetHeader>
              <div className="mt-6 space-y-4">
                <Section title="AI Analysis">
                  <div className="font-mono text-[11px] text-muted-foreground mb-1">
                    model: {sel.ai} · confidence {sel.confidence}%
                  </div>
                  <p className="text-sm text-foreground/85">
                    The endpoint reflects unsanitized input into a downstream subsystem. Observed
                    behaviour matches the {sel.module.toLowerCase()} signature with high confidence.
                  </p>
                </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="font-mono text-[10px] uppercase tracking-widest text-emerald mb-2">
        {title}
      </div>
      {children}
    </div>
